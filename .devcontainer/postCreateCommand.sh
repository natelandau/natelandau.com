...
_mainScript_() {
    ...
    if command -v batcat &> /dev/null; then
        _execute_ -p "mkdir -p ${HOME}/.local/bin && ln -s /usr/bin/batcat ${HOME}/.local/bin/bat"
    fi
    ...
    echo ""
    header "Configuring Ruby Environment"
    _execute_ -pv "bundle install"

    echo ""
    header "Configuring NodeJS Environment"
    _execute_ -pv "npm install"
    # _execute_ -pv "npm install -g commitizen"

    echo ""
    header "Configuring Python environment"
    if command -v python3 &>/dev/null; then
        _execute_ -pv "sudo apt install python3-pip -y"
        _execute_ -pv "pip install --upgrade pip"
        _execute_ -pv "pip install -U Commitizen pre-commit yamllint detect-secrets"
        _execute_ -pv "pre-commit install"
    ...
    _execute_ 'git config --global user.email "github@natenate.org"'
    _execute_ 'git config --global user.name "Nate Landau"'
